...
const Diamonds: FC<IDetailsItem> = forwardRef(({ attributes, setDetails, isSuggestProduct, productData }, ref) => {
  ...
  } = useFormValidation(NaturalDiamondsDtoValidator);
  ...
  useEffect(() => {
    if (Object.keys(productData.attributes).length > 0) {
      ...
      // setValue(Field.Grown, productData.attributes.grown);
  ...
  return (
    <form ref={formRef} onSubmit={handleForm(handleData)} noValidate autoComplete="off">
      ...
      {/*<Select*/}
      {/*  placeholder={tr.get('manageProduct.Fields.SpecificAttributes.Grown')}*/}
      {/*  defaultText={tr.get('manageProduct.Fields.Select')}*/}
      {/*  error={errors(Field.Grown)}*/}
      {/*  options={arrayForSelect(attributes.grown)}*/}
      {/*  defaultValue={productData.attributes?.grown}*/}
      {/*  onChange={({ value }) => {*/}
      {/*    setValue(Field.Grown, value);*/}
      {/*    trigger(Field.Grown);*/}
      {/*  }}*/}
      {/*  isSuggestProduct={isFieldSuggested(Field.Grown, productData, isSuggestProduct)}*/}
      {/*/>*/}
